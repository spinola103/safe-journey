...
class RoutingService {
  ...
  static Future<List<JourneyPlan>> fetchRoutes({
    ...
    try {
      ...
      final to = destination.contains('Chennai')
          ? destination
          : '$destination, Chennai';
      ...
      _demoFallback(origin, destination);
      if (plans.length == 1) {
        return plans;
      }
  ...
  static List<Leg> _parseLegs(
    ...
    for (final rawStep in allSteps) {
      ...
      final polylineStr = (step['polyline']?['points'] ?? '') as String;
      ...
      if (step.containsKey('transit_details')) {
        ...
        startStop =
            (transit['departure_stop'] as Map)['name'] as String? ?? startStop;
        endStop =
            (transit['arrival_stop'] as Map)['name'] as String? ?? endStop;
        ...
        // Walking start — use previous leg's end stop name
        if (legs.isNotEmpty) {
          startStop = legs.last.endStop;
        } else {
          startStop = origin; // first walking step starts at origin
        }

        // Find next transit stop name instead of GPS coordinates
        String resolvedEnd = destination;

        final currentIndex = allSteps.indexOf(step);

        for (int j = currentIndex + 1; j < allSteps.length; j++) {
          final nextStep = allSteps[j] as Map<String, dynamic>;

          if (nextStep.containsKey('transit_details')) {
            final transit = nextStep['transit_details'] as Map<String, dynamic>;

            resolvedEnd =
                (transit['departure_stop'] as Map)['name'] as String? ??
                destination;
  ...
  static String _locStr(dynamic loc) {
    if (loc == null) return '';
    // Return empty — walking steps resolve names from adjacent transit steps
    return '';
  }
  ...
  static List<JourneyPlan> _sortedPlans(
    ...
    final cheapest = [...plans]
      ..sort((a, b) => a.totalFare.compareTo(b.totalFare));
    final safest = [...plans]
      ..sort((a, b) => b.avgSafetyScore.compareTo(a.avgSafetyScore));
    if (womensMode) {
      return [
        safest[0],
        cheapest.firstWhere(
  ...
  static Future<List<Leg>> getAlternativesForLeg(Leg currentLeg) async {
    try {
      final routes = await fetchRoutes(
        origin: currentLeg.startStop,
        destination: currentLeg.endStop,
        womensMode: false,
      );

      final alternatives = <Leg>[];

      for (final route in routes) {
        if (route.legs.isNotEmpty) {
          alternatives.add(route.legs.first);
        }
      }
